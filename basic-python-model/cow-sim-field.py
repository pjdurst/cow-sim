...
field = np.full((100, 100), 5, dtype=int)
# Create a 5x5 array of cow locations
cows = np.random.randint(1, 101, size=(5, 2))
...
for i in range(5):
    ...
    if field[cow_x_position-1, cow_y_position] > field[cow_x_position, cow_y_position]:
        cows[i,1] = field[cow_x_position-1, 1]
        cows[i,2] = field[cow_y_position, 2]
    if field[cow_x_position+1, cow_y_position] > field[cow_x_position, cow_y_position]:
         cows[i,j] = field[cow_x_position+1, cow_y_position]
    if field[cow_x_position, cow_y_position-1] > field[cow_x_position, cow_y_position]:
         cows[i,j] = field[cow_x_position, cow_y_position-1]    
    if field[cow_x_position, cow_y_position+1] > field[cow_x_position, cow_y_position]:
        cows[i,j] = field[cow_x_position, cow_y_position+1]
    else:
